...
public class TimeClock extends View {
	...
	private void initPaint() {
		...
		paintNum.setTextAlign(Paint.Align.CENTER);//设置对齐方式
		//秒针
	...
	private void initCurrentTime(Canvas canvas) {
		...
		int hour = Integer.parseInt(split[0]);//转换为整型
		int minute = Integer.parseInt(split[1]);
		int second = Integer.parseInt(split[2]);
		// 时针走过的角度（分针走30分针，时针停在整点之间）
		int hourAngle = hour * 30 + minute / 2;
		// 分针走过的角度（秒针走半圈，分针走一半刻度）
